...
- **`opencode.json`, `oh-my-openagent.json`, `tui.json`, and `~/.omo/omo.jsonc`** — synced (including `.json` / `.jsonc` variants), with a local-only `opencode-sync.overrides.jsonc` for per-machine tweaks.
