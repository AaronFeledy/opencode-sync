...
- **Legacy payload migrate is chunked and disk-aware.** It converts at most `OPENCODE_SYNC_MIGRATE_CHUNK_ROWS` (default 500) rows, pauses if data-dir free space is below 1 GiB (or on ENOSPC), persists a `legacy_migrate_rowid` cursor in `server_state`, and resumes on the next chunk. It does **not** VACUUM; NULLing TEXT does not shrink `ledger.sqlite` until you have room for `VACUUM` / `VACUUM INTO`.
