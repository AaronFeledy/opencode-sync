...
- **Legacy payload migrate is chunked and disk-aware.** It converts at most `OPENCODE_SYNC_MIGRATE_CHUNK_ROWS` (default 500) rows, pauses if data-dir free space is below 1 GiB (or on ENOSPC), persists a `legacy_migrate_rowid` cursor in `server_state`, and resumes on the next chunk. New ledgers use `PRAGMA auto_vacuum=INCREMENTAL` and run `incremental_vacuum` after each chunk so NULLed TEXT returns to the OS. Existing ledgers created with `auto_vacuum=NONE` (starship’s 12G file) cannot punch holes; their freelist stays inside the file until a full `VACUUM`/`VACUUM INTO` that needs ~used-page bytes free.
- **opencode.db has no index on `time_updated`.** `SELECT … WHERE time_updated > ?` on `part`/`message` is a full scan (30 s+ cold on a 10 GB DB). Startup push uses `iterateAllEnvelopes(..., { sessionScoped: true })`, which goes through the `session_id` indexes for sessions updated in the window. Only the background/periodic full sync should do the unscoped scan.
